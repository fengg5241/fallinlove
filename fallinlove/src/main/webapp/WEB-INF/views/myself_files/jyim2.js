...
(function($, doc){
	...
	{
		this._id = id;
		this._dom = $('<div id="' + id + '" class="im_win"><div class="pr im_win_title"><div class="title_leftBg fl"></div><h1 class="title_centerBg"></h1><div class="title_rightBg fl"></div></div><div class="im_list"></div></div>');
	...
	var is_profile = (common_bot_sub_domain == 'profile') || /^(http\:\/\/www\.jiayuan\.com\/\d+\/?)([\?\#])?.*$/.test(document.location.href) || /^(http\:\/\/jiayuan\.msn\.com\.cn\/\d+\/?)([\?\#])?.*$/.test(document.location.href) || /^(http\:\/\/www\.miuu\.cn\/\d+\/?)([\?\#])?.*$/.test(document.location.href);
	...
	if(im_switch)
	{
		...
		if(!old_pop_content) old_pop_content = pop_content;
	...
	function izhenxinClick()
	{	

		reportPV("im_izhenxin_click|" + mysex + "|" + myuid);
		
		/*var head = document.getElementsByTagName('head')[0];
		var script = document.createElement('script');
		script.type = 'text/javascript';
		script.src = 'http://www.izhenxin.com/login/getSign/';
	...
	function closeSession()
	{
		...
		if(im_switch)
		{
			pop_content = old_pop_content;
			_mini = old_mini;
			insert_con = old_insert_con;
			...
			$.ajax({
				...
				success: function(info){
					...
					if(info.nps){
						...
						$.getScript('http://ads.jiayuan.com/ad.php?pd_id=10', function(){
							buildExFootbar(info);
						});	
					}
				}
	...
	function get_izhenxin_info(step){
		var is_bind = ImGetCookie('IZ_bind'+myuid);
		if(is_bind=='' || is_bind == 1 || step == 2){
			$.ajax({
				type: "GET",
				url: getAjaxUrl("ajax.php?svc=get_izhenxin_info"),
				dataType: "jsonp",
				jsonp:'jsoncallback',
				success: function(info){
					if(is_bind == '') ImSetCookie_v2('IZ_bind'+myuid,info.is_bind,60);
					if(info.is_bind == 1){
						if(info.need_active && info.msg==0){
							izhenxin_btn = new JyFootbarBtnItem('激活爱真心帐号', '', 'right', izhenxinClick);
							izhenxin_btn.addItemClass("izhenxin2");
							izhenxin_btn.attr('href',' http://www.izhenxin.com/login/getSign/?url=http%3A%2F%2Fwww.jiayuan.com%2Fusercp%2Fiz_login.php%3Furl%3Dhttp%253A%252F%252Fwww.izhenxin.com%252F&from=bottom');
							izhenxin_btn.attr('target','_blank');
							footbar.addBtnItem(izhenxin_btn);
						}else{
							if(step == 1) izhenxin_btn = new JyFootbarBtnItem('爱真心消息', '', 'right', izhenxinClick);
							if(info.msg>0 || info.msg=='99+'){
								izhenxin_btn.addItemClass("izhenxin");	
								izhenxin_btn.removeItemClass("izhenxin1");	
							}else{
								izhenxin_btn.addItemClass("izhenxin1");	
								izhenxin_btn.removeItemClass("izhenxin");
							}
							if(step == 1){
								izhenxin_btn.addNumber(info.msg);	
								if(info.need_active){
									izhenxin_btn.attr('href',' http://www.izhenxin.com/login/getSign/?url=http%3A%2F%2Fwww.jiayuan.com%2Fusercp%2Fiz_login.php%3Furl%3Dhttp%253A%252F%252Fwww.izhenxin.com%252F&from=bottom');
								}else{
									izhenxin_btn.attr('href','http://www.izhenxin.com/login/getSign/?url='+im_root_url+'usercp/iz_login.php&from=bottom');
								}
								izhenxin_btn.attr('target','_blank');
							}else{
								izhenxin_btn.changeNumber(info.msg);
							}
							if(info.msg > 0 || info.msg=='99+'){
								izhenxin_btn.showNumber();	
							}else{
								izhenxin_btn.hideNumber();	
							}
							if(step == 1){
								footbar.addBtnItem(izhenxin_btn);		
	...
	function buildExFootbar(info){
		...
		if (typeof(publish_webim_ad) !== 'undefined'){
			...
			var mid_btn = new JyFootbarBtnItem(webim_ad1[0], '', 'left',
				function(){
		...
		if(typeof(publish_webim_ad) !== 'undefined'){
			...
			for(i=1;i<len_ads;i++){
				...
				var mid_btn = new JyFootbarBtnItem(webim_ad[0], '', 'left',af.bindfun);
				$(".im_inner_btn", mid_btn.getDom()).css("width", "300px");
				mid_btn.addItemClass("brand_mid not_hover");
				mid_btn.getDom().hide();
				footbar.addBtnItem(mid_btn);
