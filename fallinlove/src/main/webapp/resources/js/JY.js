            ...
            if (response != "" && response.length < 6e3) {
                ...
                if (_this.frameStatus == 1) {
                    ...
                    if (user.uid < 1) {
                        ...
                        param.uid = user.uid, param.sex = user.sex, param.nickname = JY.tool.cookie.unCharCode(decodeURIComponent(user.nickname)), param.age = user.age, param.work_location = user.work_location, param.work_sublocation = user.work_sublocation;
                        try {
